...
public class GUI extends javax.swing.JFrame {
    ...
    public void createGraphUI() {
        ...
        int qtdVertex = 0;
        if (graph != null) {
            qtdVertex = graph.getListVertex().size();
        }
        if (qtdVertex == 0) {
            qtdVertex = 1;
        }
        int step = 360 / qtdVertex;
        int i = 0;
        int angle = 0;
        ...
        if (graph != null) {
            ...
            for (Edge edge : graph.getListEdge()) {
                Vertex vertex1 = graph.getListEdge().get(graph.getListEdge().indexOf(edge)).getVertex1();
                Vertex vertex2 = graph.getListEdge().get(graph.getListEdge().indexOf(edge)).getVertex2();
    ...
    private void initComponents() {
        ...
        jCbSelectAlgorithm.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "DFS", "BFS", "Dijkstra" }));
        ...
        jLabel6.setText("ORDEM DE VISITADOS");
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 0;
        gridBagConstraints.gridy = 2;
        gridBagConstraints.fill = java.awt.GridBagConstraints.BOTH;
    ...
    private void jBtnAddEdgeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBtnAddEdgeActionPerformed
        if (this.graph.getPondered()) {
            if (!jTxtAddEdgeWeight.getText().isEmpty() && !jTxtAddEdgeVertex1.getText().toString().isEmpty() && !jTxtAddEdgeVertex2.getText().toString().isEmpty()) {
                if (Integer.valueOf(jTxtAddEdgeWeight.getText()) > 0) {
                    ...
                    if (!(graph.getVertexByLabel(labelVertex1) == null) && !(graph.getVertexByLabel(labelVertex2) == null)) {
                        Edge edge = new Edge(graph.getVertexByLabel(labelVertex1), graph.getVertexByLabel(labelVertex2), labelWeight);
        ...
        } else if (!jTxtAddEdgeVertex1.getText().toString().isEmpty() && !jTxtAddEdgeVertex2.getText().toString().isEmpty()) {
            String labelVertex1 = jTxtAddEdgeVertex1.getText();
            String labelVertex2 = jTxtAddEdgeVertex2.getText();
            if (!(graph.getVertexByLabel(labelVertex1) == null) && !(graph.getVertexByLabel(labelVertex2) == null)) {
                Edge edge = new Edge(graph.getVertexByLabel(labelVertex1), graph.getVertexByLabel(labelVertex2));
                this.graph.addEdge(edge);
                jTxtAddEdgeVertex1.setText("");
                jTxtAddEdgeVertex2.setText("");
            } else {
        ...
        for (AdjacencyList adjacencyList : graph.getAdjacencyList()) {
            jTxtAdjacencyList.setText(jTxtAdjacencyList.getText() + adjacencyList.getVertex().getLabel() + " -> ");
            for (Vertex vertex : adjacencyList.getVertexList()) {
    ...
    private void createGraph() {
        this.graph = new Graph(jCbOriented.isSelected(), jCbPondered.isSelected());
        this.vertexLabelList = new ArrayList<>();
        jLblVertexList.setText("");
        jTxtVisitedList.setText("");
        jTxtAdjacencyList.setText("");
